//
//  ethminer_metal.metal
//  ethminer-metal
//
//  Created by yiwei yang on 2021/7/6.
//
...
const uint2 constant Keccak_f1600_RC[24] = {
...
static void chi(device uint2 * a, const uint n, device const uint2 * t)
{
...
static void keccak_f1600_round(device uint2* a, uint r)
{
    uint2 t[25];
    uint2 u;
...
static void keccak_f1600_no_absorb(device uint2* a, uint out_size, uint isolate)
{
...
static void SHA3_512(device uint2* s, uint isolate)
{
    for (uint i = 8; i != 25; ++i)
    {
        s[i] = (uint2){ 0, 0 };
